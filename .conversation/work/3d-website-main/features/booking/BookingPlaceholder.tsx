...
export function BookingPlaceholder() {
  ...
  return <main className="booking-page min-h-screen bg-bg-void px-4 pb-20 pt-28">
    <div className="mx-auto max-w-5xl">
      ...
      <div className="booking-steps mb-8 grid grid-cols-4 gap-1 md:grid-cols-8">{steps.map((label, index) => <button key={label} onClick={() => index <= step && setStep(index)} className={`group flex flex-col items-center gap-2 text-center text-[10px] font-devanagari transition-colors duration-300 ${index <= step ? 'text-gold' : 'text-text-muted/50'}`}><motion.span animate={{ scale: 1, boxShadow: 'none' }} transition={{ duration: 0.35, ease: EASE_PREMIUM }} className={`flex h-8 w-8 items-center justify-center rounded-full border text-xs transition-colors duration-300 ${index < step ? 'border-gold bg-gradient-to-br from-gold-warm to-gold text-bg-void' : index === step ? 'border-gold bg-gold/10 text-gold' : 'border-text-muted/30'}`}>{index < step ? <Check size={14} /> : index + 1}</motion.span>{label}</button>)}</div>
      <AnimatePresence mode="wait" initial={false}>
      <motion.section key={step} initial={false} animate={{ opacity: 1 }} exit={reduce ? undefined : { opacity: 0 }} transition={{ duration: reduce ? 0 : 0.12 }} className="booking-panel rounded-3xl border border-gold/15 bg-gradient-to-br from-bg-purple/80 to-bg-rich/80 p-5 shadow-card-lift md:p-10 backdrop-blur-sm">
